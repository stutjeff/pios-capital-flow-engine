...
class EodhdBackend(MarketBackend):
    def fetch(self,symbol,column,ctx,used,exchange='US'):
        ...
        source=f'EODHD:{ticker}'
        endpoint=f'https://eodhd.com/api/eod/{ticker}'
        fmt='GET api_token,fmt=json,from,to,period=d,order=a'
        key=env('EODHD_API_KEY')
        if not key:
            df,items=missing(source,'MARKET_DATA','EODHD_API_KEY',endpoint,fmt,'YES',used)
            return df,items[0]
        ...
        return df,result_status(source,'MARKET_DATA',df,requires_key=True,secret_name='EODHD_API_KEY',history_supported='YES',used_in_model=used,endpoint=endpoint,fmt=fmt)
